...
def setup():
	global  motor_change_num, current_l, sonar_dis, x_cord, y_cord
...
def findCloseObject():
	...
	for i in range(len(sonar_dis)):
		if min_dis > sonar_dis[i]:
			min_dis = sonar_dis[i]
			min_dis_channel = i
	object_loc = 45*min_dis_channel
	print "the nearest object is ", object_loc
	if min_dis > edge_dis:
		object_loc = right_angle
	print "The nearest object is ",  object_loc
	return object_loc

def avoidanceTurn():
	
	if (weighted_angle_avg > 112) & (weighted_angle_avg < 270):	
		accelerate(0, 0.5)
		print "turning counterclockwise"
	elif (weighted_angle_avg < 68 )  | (weighted_angle_avg > 270):
		accelerate(0, -0.5)
		print "turning clockwise"
#		print weighted_angle_av
	avgCord()
...
def motion(): #main driving function 
	print ('Driving')
	avgCord()
	if sonar_dis[f_sonar] < tooCloseDis:
